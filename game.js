...
// DARKSIDE - updated: fixed START/BOOST wiring, single main loop, proper rocket sprite, bullets spawn from rocket nose
// Visuals (starfield, nebula, enemy sprites) are preserved.
...
let running = false;           // gameplay active (updates)
let lastTime = now();
let seed = Math.random()*9999;
let mainLoopStarted = false;   // ensures single requestAnimationFrame loop
...
// NEW: clear rocket-style player SVG (pointed nose at top, cockpit, fins, engine)
function playerSVG(){
  return `
  <svg xmlns="http://www.w3.org/2000/svg" width="240" height="300" viewBox="0 0 240 300">
    <defs>
      <linearGradient id="bodyG" x1="0" x2="1">
        <stop offset="0" stop-color="#1f2633"/>
        <stop offset="0.5" stop-color="#2b3442"/>
        <stop offset="1" stop-color="#101218"/>
      </linearGradient>
      <linearGradient id="noseG" x1="0" x2="1">
        <stop offset="0" stop-color="#aee9ff"/>
        <stop offset="1" stop-color="#62c8ff"/>
      </linearGradient>
      <radialGradient id="engineG" cx="50%" cy="50%">
        <stop offset="0" stop-color="#aaf"/>
        <stop offset="0.4" stop-color="#39f"/>
    ...
    <g transform="translate(120,160)">
      <!-- nose cone -->
      <path d="M0 -120 C 12 -110 44 -86 44 -56 C 44 -36 24 -12 0 -12 C -24 -12 -44 -36 -44 -56 C -44 -86 -12 -110 0 -120 Z"
        fill="url(#bodyG)" stroke="#0e1420" stroke-width="2"/>
      <!-- cockpit window -->
      <ellipse cx="0" cy="-46" rx="18" ry="12" fill="rgba(160,250,255,0.95)" opacity="0.95" stroke="#0b1820" stroke-width="1.5" filter="url(#softGlow)"/>
      <ellipse cx="0" cy="-48" rx="10" ry="6" fill="#d9fbff" opacity="0.28"/>
      <!-- main body -->
      <path d="M-34 -8 C -12 40 -6 78 0 120 C 6 78 12 40 34 -8 L 34 6 C 12 48 0 110 0 140 C 0 170 24 170 48 170 C 24 186 0 190 0 212 C 0 234 8 250 0 260 C -8 250 0 234 0 212 C 0 190 -24 186 -48 170 C -24 170 0 170 0 140 C 0 110 -12 48 -34 6 Z"
        fill="url(#bodyG)" opacity="0.98"/>
      <!-- left fin -->
      <path d="M-40 8 L -86 44 L -58 18 Z" fill="#202633" />
      <!-- right fin -->
      <path d="M40 8 L 86 44 L 58 18 Z" fill="#202633" />
      <!-- small fins near tail -->
      <path d="M-26 110 L -54 130 L -36 116 Z" fill="#222732" />
      <path d="M26 110 L 54 130 L 36 116 Z" fill="#222732" />
      <!-- engine cluster -->
      <g transform="translate(0,170)">
        <ellipse rx="26" ry="12" fill="#081018"/>
        <ellipse rx="14" ry="6" fill="url(#engineG)" opacity="0.98" />
        <!-- engine glow -->
        <g transform="translate(0,28)">
          <ellipse rx="22" ry="10" fill="rgba(140,220,255,0.18)"/>
        </g>
      </g>
      <!-- panel lines -->
      <path d="M0 -92 C 14 -82 34 -56 44 -28" stroke="#0f1a25" stroke-width="1.2" fill="none" opacity="0.22"/>
      <path d="M0 -92 C -14 -82 -34 -56 -44 -28" stroke="#0f1a25" stroke-width="1.2" fill="none" opacity="0.22"/>
    </g>
  </svg>
  `;
...
// enemySVG unchanged (use prior enemySVG implementations)
function enemySVG(type){
  if(type==='scout'){
    return `
    <svg xmlns="http://www.w3.org/2000/svg" width="160" height="160" viewBox="0 0 160 160">
...
function createPlayer(){
  const p = {
    ...
    boosting: false,
    boostEnd: 0,
    // nose offset: how far from center is the rocket tip (pixels)
    // this should match the rocket SVG sizing + the draw scale used when rendering (we use size 84)
    noseOffset: 52,   // tuned for the rocket SVG so bullets appear at the nose
    spriteSize: 84
  };
  state.player = p;
}
...
function spawnEnemy(){
  ...
  const margin = 60;
  const x = rand(margin, state.w - margin);
  const startY = -rand(120, 420); // above screen (varied)
  let speed = t==='scout' ? rand(120,200) : t==='fighter' ? rand(80,140) : rand(40,90);
  speed *= 1 + (state.level-1)*0.08;
  ...
  const enemy = {
    ...
    enterDuration: rand(700,1400),
    enterFromY: startY,
    enterTargetY: rand(30, Math.max(80, state.h * 0.34)),
    entering: true
  };
  state.enemies.push(enemy);
}
...
function initUI(){
  ...
  // ensure main loop is running once
  startMainLoop();
}

// Start button handler (keeps single handler and avoids duplicates)
function onStartClicked(){
  // resume audio if suspended by browser
  if(audioCtx && audioCtx.state === 'suspended') audioCtx.resume();
  // start a fresh game (use restart which resets game state)
  restart();
}
...
function activateBoost(){
  ...
  if(!p) return;
  const nowt = now();
  // allow boost only when energy sufficient
  if(state.energy > 12){
    p.boosting = true;
    p.boostEnd = nowt + 1400; // ms
...
function loop(){
  ...
  // Only update game simulation when running is true
  if(running){
    update(dt);
  } else {
    // Even when not running we update HUD values visually (energy/shield) so buttons show correct state
    // This keeps overlay UI responsive. We still call draw-only rendering below.
    // minimal passive regeneration so HUD reflects expected values
    if(state.player){
      state.energy = clamp(state.energy + dt*3, 0, 100);
    }
    drawRadar(); // keep radar updated for UI polish
  }

  render(); // always render so overlay and HUD show correctly
  requestAnimationFrame(loop);
}
...
function update(dt){
  ...
  // Keep player in lower portion of the screen
  const topLimit = state.h * 0.58;   // do not allow moving above this
  p.x = clamp(p.x, 30, state.w-30);
  p.y = clamp(p.y, topLimit, state.h-30);

  // player rotation toward mouse, but bias upward
  const mx = state.mouse.x, my = state.mouse.y;
  const angleToMouse = Math.atan2(my - p.y, mx - p.x);
  let targetAngle;
  const dmouse = Math.hypot(mx - p.x, my - p.y);
  ...
  // Enemies update (entrance animation and downward movement)
  for(let e of state.enemies){
    if(e.entering){
      const elapsed = now() - e.enterStart;
      const tProg = clamp(elapsed / e.enterDuration, 0, 1);
      // ease out cubic
      const ease = 1 - Math.pow(1 - tProg, 3);
      e.y = e.enterFromY + (e.enterTargetY - e.enterFromY) * ease;
      e.x += e.vx * dt * 0.6;
      if(now() - e.enterStart >= e.enterDuration) {
        e.entering = false;
  ...
  // Remove enemies that leave bottom
  state.enemies = state.enemies.filter(e => !(e.y > state.h + 220) && e.hp > -300);
...
// Shooting - bullets originate from rocket nose
function shootPlayer(){
  const p = state.player;
  const baseAngle = p.rotation; // points upward by default
  // compute nose coordinate
  const noseX = p.x + Math.cos(baseAngle) * p.noseOffset;
  const noseY = p.y + Math.sin(baseAngle) * p.noseOffset;
  for(let off of [-0.02, 0.02]){
    const angle = baseAngle + off;
    const speed = 780;
    state.bullets.push({
...
function render(){
  ...
  // Draw enemies (preserve entrance alpha/scale)
  for(let e of state.enemies){
    ctx.save();
    ctx.translate(e.x, e.y);
    ctx.rotate(e.rot);
  ...
  // Draw player with engine glow and rocket sprite; bullet origin is at noseOffset
  const pl = state.player;
  if(pl){
    ctx.save();
    ctx.translate(pl.x, pl.y);
    ...
    // engine glow at tail (preserve original glow feel)
    ctx.save();
    ctx.globalCompositeOperation = 'lighter';
    const g2 = ctx.createRadialGradient(-6, 68, 2, 0, 80, 120);
    g2.addColorStop(0, 'rgba(140,220,255,0.9)');
    ...
    // ship draw (SVG image)
    if(playerSprite){
      const size = pl.spriteSize || 84;
      ctx.drawImage(playerSprite, -size/2, -size/2 - 8, size, size + 20); // slight vertical offset to match nose
    }

    // muzzle flare at nose when shooting
    if(state.mouse.down){
      const noseX = Math.cos(pl.rotation) * pl.noseOffset;
      const noseY = Math.sin(pl.rotation) * pl.noseOffset;
      ctx.save();
      ctx.globalAlpha = 0.22 + Math.random()*0.16;
      ctx.fillStyle = '#aef2ff';
      ctx.beginPath();
      ctx.ellipse(noseX, noseY, 10, 4, 0, 0, Math.PI*2);
      ctx.fill();
      ctx.restore();
    }
...
function drawRadar(){
  ...
  const radius = state.player ? state.player.radarRadius : Math.min(state.w,state.h)*0.25;
  for(let e of state.enemies){
    const dx = e.x - (state.player ? state.player.x : state.w/2);
    const dy = e.y - (state.player ? state.player.y : state.h/2);
    if(Math.abs(dx) > radius || Math.abs(dy) > radius) continue;
    const rx = (dx/radius) * (s/2) + s/2;
    const ry = (dy/radius) * (s/2) + s/2;
    if(e.revealed){
...
// Restart (reset state and begin playing)
function restart(){
  state.bullets.length = 0;
  state.enemyBullets.length = 0;
  state.enemies.length = 0;
...
// initial spawn and ensure radar draws
state.spawnTimer = 500;
for(let i=0;i<2;i++) spawnEnemy();
